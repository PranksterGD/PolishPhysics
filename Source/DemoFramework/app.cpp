...
void RigidBodyApplication::drawDebug()
{
    ...
    glBegin(GL_LINES);
    for (unsigned i = 0; i < cData.mContactsFound; i++)
    {
        // Interbody contacts are in green, floor contacts are red.
        if (contacts[i].mBodies[1]) {
            glColor3f(0,1,0);
        } else {
            glColor3f(1,0,0);
        }

        Vector3 vec = contacts[i].mContactPosition;
        glVertex3f(vec.X, vec.Y, vec.Z);

        vec += contacts[i].mContactNormal;
        glVertex3f(vec.X, vec.Y, vec.Z);
    }
